...
export default class TodoApp {
  ...
  stopEditing = ev => {
    ...
    if (text) {
      store.dispatch(todoAppActions.stopEditing(key, text));
    } else {
      store.dispatch(todoAppActions.removeTodo(key));
    }
  }
  ...
  handleKeyDown = ev => {
    ...
    switch (ev.keyCode) {
      case 27: {
        ...
        ev.target.value = todoApp.todos.find(todo => todo.key === key).title;
