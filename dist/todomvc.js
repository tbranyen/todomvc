 ...
 * nested children.
 */
function inlineTransitions(options) {
  ...
  var attributeChanged = function (domNode, name, oldVal, newVal) {
  ...
  var subscribe = function () {
    ...
    keys$1(transitionsMap).forEach(function (name) {
      var map = transitionsMap[name];
      ...
      var handler = function (child) {
        ...
        if (map.has(child)) {
          return map.get(child).apply(void 0, [ child, child ].concat( rest ));
        }
        ...
        // `Promise.all`.
            ...
            var hasPromise = retVal.some(function (ret) { return Boolean(ret && ret.then); });
  ...
  return assign$2(function inlineTransitionsTask() {}, { subscribe: subscribe, unsubscribe: unsubscribe });
}
		...
		case TOGGLE_COMPLETION:
			{
				...
				state.todos[index] = assign$8({}, todo, {
		...
		case START_EDITING:
			{
				...
				state.todos[_index] = assign$8({}, _todo, {
					editing: true
				});
		...
		case STOP_EDITING:
			{
				...
				state.todos[_index2] = assign$8({}, _todo2, {
					title: action.title,
					editing: false
				});
...
function url() {
	...
	switch (action.type) {
		case SET_HASH_STATE:
			{
				return assign$9({}, state, { path: action.path });
			}
